...
pub fn main() !void {
    ...
    var copies = std.ArrayList(u64).init(a);
    defer copies.deinit();
    try copies.append(1);
    var copy_i: usize = 0;
    ...
    while (line_it.next()) |line| {
        ...
        while (it.next()) |str| {
            ...
            if (std.mem.indexOfScalar(u64, wins.items, num)) |_| count += 1;
        }

        if (copies.items.len - copy_i < count + 1)
            try copies.appendNTimes(1, copy_i + count + 1 - copies.items.len);
        sum += copies.items[copy_i];
        for (copy_i + 1..copy_i + 1 + count) |i| copies.items[i] += copies.items[copy_i];
        copy_i += 1;
    }

...
pub fn main() !void {
    ...
    const Map = std.AutoArrayHashMap(struct { x: u8, y: u8, dir: Dir, limit: u2 }, u32);
    ...
    while (queue.removeOrNull()) |prev_index| {
        ...
        if (prev.x == width - 1 and prev.y == height - 1) {
            std.debug.print("{}\n", .{prev_loss});
            break;
        }
        for (std.enums.values(Dir)) |dir| {
            ...
            const gop = try visited.getOrPut(.{
                ...
                .y = @intCast(next.y),
                .dir = dir,
                .limit = if (dir == prev.dir) std.math.sub(u2, prev.limit, 1) catch continue else 2,
            });

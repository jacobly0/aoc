...
pub fn main() !void {
    ...
    while (line_it.next()) |line| {
        ...
        try list.items[i].append(0);
        while (i > 0) : (i -= 1) {
            try list.items[i - 1].append(list.items[i - 1].items[list.items[i - 1].items.len - 1] +
                list.items[i].items[list.items[i].items.len - 1]);
        }

        sum += list.items[0].items[list.items[0].items.len - 1];
    }
    std.debug.print("{}\n", .{sum});
}

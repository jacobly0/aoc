...
pub fn main() !void {
    ...
    while (line_it.next()) |line| {
        ...
        if (std.sort.isSorted(u32, update.items, &map, lessThan)) continue;
        std.mem.sortUnstable(u32, update.items, &map, lessThan);
        sum += update.items[update.items.len / 2];
    }
    try stdout.print("{d}\n", .{sum});
}
fn lessThan(map: *const std.AutoHashMapUnmanaged(struct { u32, u32 }, void), lhs: u32, rhs: u32) bool {
    return map.contains(.{ lhs, rhs });
}
